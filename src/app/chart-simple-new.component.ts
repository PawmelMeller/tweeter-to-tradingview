...
import { BitcoinService, BitcoinPriceData } from './bitcoin.service';
  ...
  template: `
    <div class="chart-container">
      ...
      <div class="chart-footer" *ngIf="bitcoinData.length > 0">
        <small>
          Data source: Binance API • Updated every 5 minutes
...
export class ChartComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  loadBitcoinData() {
    ...
    this.bitcoinService.getBitcoinPriceHistory(24).subscribe({
      next: (data) => {
    ...
    this.updateSubscription = interval(5 * 60 * 1000).subscribe(() => {
      this.bitcoinService.getCurrentBitcoinPrice().subscribe({
        next: (price) => {
          ...
          // Keep only last 24 hours of data (24 points for hourly data)
          if (this.bitcoinData.length > 24) {
            this.bitcoinData = this.bitcoinData.slice(-24);
          }
